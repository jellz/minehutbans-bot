...
const run = module.exports.run = async (msg, args) => {
  ...
  if (args.slice(1).join(' ').length > 1000) {
    msg.delete({ reason: 'invalid report command usage' });
    let m = await msg.channel.send(client.config.emoji.fail + ' Your report is too long.');
  ...
  if (uuidRes.status == 204) { // not a real player
    msg.delete({ reason: 'invalid report command usage' });
    let m = await msg.channel.send(client.config.emoji.fail + ' The player you have specified is not real.');
    return m.delete({ timeout: 5000 });
  }
  ...
  let m = await msg.channel.send(client.config.emoji.success);
  return m.delete({ timeout: 5000 });
}
